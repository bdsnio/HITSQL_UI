...
function updateFriendButton() {
    // 从localStorage获取好友请求列表
    friendRequestSendReceiveList = JSON.parse(localStorage.getItem('friendRequestSendReceive')) || [];

    // 遍历所有加为好友按钮，恢复状态
    document.querySelectorAll('.friend-btn').forEach(function(friendBtn) {
        var post = friendBtn.closest('.post');
        var postUsername = post.querySelector('.post-username').textContent;

        // 检查是否已经发送了好友请求
        var isFriendRequested = friendRequestSendReceiveList.some(function(request) {
            return request[0] === postUsername && request[2] === username;
        });

        if (isFriendRequested) {
            friendBtn.innerHTML = '已成为好友';
            friendBtn.style.backgroundColor = '#f0f0f0'; // 浅蓝色背景
            friendBtn.style.color = '#333'; // 黑色文字
            friendBtn.style.fontWeight = 'normal'; // 字体改为非加粗
        } else {
            friendBtn.innerHTML = '加为好友';
            // 可以根据需要设置未发送好友请求时的样式
        }
    });
}
...
function updateLikeButton(){
    // 从localStorage获取点赞列表
    clickReceiveList = JSON.parse(localStorage.getItem('clickReceive')) || [];
    // 遍历所有点赞按钮，恢复状态
    document.querySelectorAll('.like-btn').forEach(function(likeBtn) {
        var post = likeBtn.closest('.post');
        var postUsername = post.querySelector('.post-username').textContent;
        var blogID = post.id;
        var isLiked = clickReceiveList.some(function(subArray) {
            return subArray[0] === postUsername && subArray[1] === username && parseInt(subArray[2]) === parseInt(blogID);
        });

        if (isLiked) {
            likeBtn.style.backgroundColor = '#4a77d4';
...
function updateCommentsDisplay() {
    // 从localStorage获取评论列表
    commentReceiveList = JSON.parse(localStorage.getItem('commentReceive')) || [];
    // 遍历所有博客帖子元素
    document.querySelectorAll('.post').forEach(function(post) {
        var postComments = commentReceiveList.filter(function(comment) {
            // 假设评论数据中包含博客ID作为评论的标识
            return parseInt(comment[4]) === parseInt(post.id);
        });
        var commentsSection = post.querySelector('.comments-content');
        commentsSection.innerHTML = ''; // 清空现有评论

        // 为每个评论创建元素并添加到评论区
        postComments.forEach(function(comment) {
        // 将评论显示在页面上
        displayComment(comment[3], comment[2], commentsSection);
...
function LikeButtonClick(event) {
    ...
    // 获取按钮的父元素，即.post元素
    var postcloset = likeBtn.closest('.post');

    // 获取blogID
    var blogID = postcloset.id;
    var postUsername=postcloset.querySelector('.post-username').textContent;
...
function ShareButtonClick(event){
    ...
    var myShareNewBlog=[username,formattedDate,postContent,postBlogID];        
...
function CommentButtonClick(event) {
    ...
    // 检查评论是否为空或只包含空白字符
        ...
        //获取点击时的格式化时间
        var formattedDate = generateFormatTime(new Date());
        var postid=postclosest.id;
        //初始化新的评论数组
        var commentList_new=[postUsername,formattedDate,commentText,username,postid];
